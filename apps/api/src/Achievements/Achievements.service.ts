...
export class AchievementsService {
  ...
  private readonly achivementsList: {
    title: string;
    description: string;
    logo: number;
  }[] = [];
  ...
  async helloAchievements(winner: {
    user: User;
    history: MatchFront[];
    ...
    if (num_of_victory === 1) {
      ...
      if (
        winner.achievements.findIndex(
          (e) => e.title === this.achivementsList[0].title,
        ) === -1
      )
  ...
  async gradeAchievements(winner: {
    user: User;
    history: MatchFront[];
    achievements: Achievements[];
  }) {
    ...
    if (num_of_victory % 2 == 0 && num_of_victory <= 10) {
      ...
      newAchievements.description =
        this.achivementsList[num_of_victory / 2].description;
      newAchievements.user = winner.user;
      newAchievements.logo = this.achivementsList[num_of_victory / 2].logo;

      if (
        winner.achievements.findIndex(
          (e) => e.title === this.achivementsList[num_of_victory / 2].title,
        ) === -1
      )
        await this.AchievementsRepository.save(newAchievements);
    }
  ...
  async forceAwaken(winner: {
    ...
    if (num_of_defeat > 3) {
      ...
      if (
        winner.achievements.findIndex(
          (e) => e.title === this.achivementsList[6].title,
        ) === -1
      ) {
        await this.AchievementsRepository.save(newAchievements);
      }
  ...
  async needTraining(looser: {
    ...
    if (num_of_defeat > 3) {
      ...
      if (
        looser.achievements.findIndex(
          (e) => e.title === this.achivementsList[7].title,
        ) === -1
      )
        await this.AchievementsRepository.save(newAchievements);
    }
  ...
  async saveAchievement(
    winner: User,
    looser: User,
    winnerHistory: MatchFront[],
    looserHistory: MatchFront[],
  ) {
    ...
    this.needTraining({
      user: looser,
      history: looserHistory,
      achievements: looserAchievements,
    });
  }
}
