...
export default function ManageConversation( props : manageConversationProps): JSX.Element
{
    ...
    return (
        <>
        { (props.self.current && props.self.current.role === ConversationRoleEnum.OWNER && props.participants.length > 1 ) ? <article>Leave (pick a new owner if you want to leave)</article> : <article onClick={() =>{
        ...
        <article onClick={ () => {setDisplaySearchBox(true)} }>Invite someone</article>
        { displaySearchBox ? (<>
                ...
                fetch(`/api/search?letters=${e.target.value}`, {
                        ...
                        response.json().then((data : User[]) => {
                            const currentMatches = data.filter((res : User) => {
                                if (participants.filter((participant) => participant.name.toLowerCase() === res.name.toLowerCase()).length)
                                    return false
                                return true
                            })
                        ...
                        .catch(function (error) {
                            setErrors( (prev) => [...prev, "Il y a eu un problème avec l'opération fetch : " + error.message])
                        });
            }} />
            <section>{ matches.map((el) => <article key={el.id} onClick={(e) =>
                {
                    ...
                    websockets.conversations.emit("inviteToConversation", {target: el.id, conversationID: props.currentConversation.id}, (res : boolean) => {
                        if (res === true)
                        {
                            setSuccess("Invitation sucessfully sent")
                            setTimeout(() => {setDisplaySearchBox(false); setSuccess(""), setErrors([])}, 2000)
                        }
                        else
                        {
                            setErrors(["Cannot invite this person in this conversation"])
                        }
