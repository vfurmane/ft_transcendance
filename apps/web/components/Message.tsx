...
export default function Message ( props : MessageProps ) : JSX.Element {
    ...
    if (props.message.system_generated)
    {
        ...
        return (
            <>
            <article className={`${styles.invitationMessage} ${styles.messageContent}`}>
                { props.message.invitation_type === InvitationEnum.CONVERSATION ? <p>You are invited to join {props.message.content}, click <span onClick={(e) => {
                    setError("")
                    setSuccess("")
                    ...
                    websockets.conversations.emit("canJoinConversation", payload, (ret : {
                        canJoin: boolean;
                        password: boolean;
                    }) =>
                    {
                        ...
                        {
                            setError("Cannot join this conversation, please verify you are not already in it or ask the invitor")
                            return
                        }
                        if (!ret.password)
                        {
                            websockets.conversations?.emit("joinConversation", { id: props.message.target }, ()=>{
                ...
                }}>here to join</span></p> : <p>Invitation to play Pong</p> }
            </article>
            { requirePassword ? <><article>Please enter password: </article>
                                    <form onSubmit={(e) => {
                                        e.preventDefault()
                                        setError("")
                                        if (!websockets.conversations?.connected)
                                        ...
                                        websockets.conversations.timeout(1000).emit("joinConversation", { id: props.message.target, password: passwordRef.current?.value }, (err : any, conversation : Conversation) =>
                                        {
                                            if (err)
                                            {
            ...
            <article ref={feedbackRef}>{error.length ? error : <></>}{success.length ? success : <></>}</article>
            </>
        )
    }
