...
export default function Conversation(props: {
  ...
  const [ isBanned, setIsBanned ] = useState<boolean>(props.conversation.conversation.conversationRoles[0].restrictions.find((restriction) => restriction.status === conversationRestrictionEnum.BAN) !== undefined)
  const websockets = useWebsocketContext();
  const BlockedUsersState = useSelector(selectBlockedUsersState);
  const [ isBlocked, setIsBlocked] = useState<boolean>(false)

  const newBanned = (banned : { conversationID : string, userId: string | undefined }) => {
    if (banned.conversationID === props.conversation.conversation.id && banned.userId !== undefined && banned.userId === userState.id)
    {  setIsBanned(true)}
  };
  ...
  const newUnbanned = (unbanned : { conversationID : string, userId: string | undefined }) => {
    if (unbanned.conversationID === props.conversation.conversation.id && unbanned.userId !== undefined && unbanned.userId === userState.id)
    {
      setIsBanned(false)
  ...
  useEffect(() => 
  {
    ...
    {
      const id = props.conversation.conversation.conversationRoles.find((role) => role.user.id !== userState.id)?.user.id
      if (id !== undefined && BlockedUsersState.includes(id) && isBlocked === false)
      {
        setIsBlocked(true)
      }
      else
