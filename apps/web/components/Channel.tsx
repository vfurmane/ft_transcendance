...
export default function Channel(props: ChannelProps) {
  ...
  const joinConversation = (err: any, conversation: ConversationEntity) => {
    if (err) {
      props.setErrors([
        "Could not join conversation, please try again later or verify password",
      ]);
  ...
  if (askPassword) {
    return (
      <section key={props.current.id} className={styles.channelForm}>
        <aside className={styles.lockedField}>
          {props.current.has_password ? (
        ...
        <section className={styles.channelInfo}>
          <article className={styles.channelName}>
          ...
          <form
            onSubmit={(e) => {
              ...
              websockets.conversations
                ...
                .emit(
                  "joinConversation",
                  { id: props.current.id, password: password },
                  joinConversation
                );
            }}
          >
            ...
            <div className={styles.joinButton}>
              <input
                type="submit"
                value="JOIN"
                className={styles.submitWithPwd}
              />
            </div>{" "}
          </form>
        </section>
  ...
  return (
    <section key={props.current.id} className={styles.channelNoForm}>
      <aside className={styles.lockedField}>
        {props.current.has_password ? (
          <Image src={lockedPadlock} width={20} height={20} alt="padlock" />
        ) : (
      ...
      <article className={styles.joinButton}>
        <p
          onClick={(e) => {
            props.setErrors([]);
            if (props.current.has_password) setAskPassword(true);
            else
              websockets.conversations
                ?.timeout(500)
                .emit(
